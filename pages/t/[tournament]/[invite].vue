    ...
    <PrimeToolbar>
      <template #start>
        <div class="flex items-center gap-2">
          ...
          <PrimeButton icon="pi pi-envelope" :label="$t('buttons.inviteLink')" @click.prevent="showInvite"
            title="Show Invite">
          </PrimeButton>
        </div>
      </template>
...
const invites = computed(() => {
  if (adjustedTournament.value.invites) {
    ...
    return inviteCodes.map((inviteId) => {
      return { id: inviteId, ...adjustedTournament.value.invites[inviteId] };
    });
  }
  return [];
});
